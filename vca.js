(function () {
	...
	function setSticky(isTop) {
		...
		// enlarge the top container to avoid breadcrumbs overlapping
			...
			for (var htmlTagsIndex = 0; htmlTagsIndex < htmlTags.length; htmlTagsIndex++) {
				...
				if (pageDetail.elementSelectorType) {
					if (!pageDetail.elementSelector) {
						console.warn('findTextContainingTags : elementSelector property missing!', pageDetail);
						continue;
					}
				...
				if (pageDetail.elementSub) {
					...
					for (var subElementsIndex = 0; subElementsIndex < subElements.length; subElementsIndex++) {
						...
						if (!subElement.innerText) {
							console.warn('findTextContainingTags : Skipping sub element (.innerText was falsy) : ', subElement);
							continue;
						}

						if (!isHtmlElementVisible(subElement)) {
							console.warn('findTextContainingTags : Skipping sub element (display property was "none") : ', subElement);
							continue;
						}
